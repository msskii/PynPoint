...
class RefLibraryMultiReader(ReadingModule):
    ...
    def read_files(self, ijk: int,
        ...
        self.m_header_out_port = self.add_output_port(
            'fits_header/'+self.m_img_tag[ijk])
    ...
    def run(self) -> None:
        ...
        for i, directory in enumerate(stars):
            ...
            header, header_dict, shape, wav_arr, chan_arr, band_arr = self.read_files(
                i, files, current_outport, overwrite_tags)
            ...
            if len(shape) == 2:
                ...
                raise ValueError(
                    'Data read from FITS file has an invalid shape.')
