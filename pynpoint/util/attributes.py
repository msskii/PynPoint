...
def set_static_attr(fits_file: str,
                    header: Union[fits.header.Header, Dict],
                    config_port: ConfigPort,
                    image_out_port: OutputPort,
                    check: bool = True,
                    instrument_key: Optional[str] = "default") -> None:
    """
    ...
    instrument_key : str
        Allows for different header information to be read in by using a different
        list of header card imports in pynpoint.core.attributes in the get_attributes function.
    ...
    attributes = get_attributes(instrument_key=instrument_key)
    ...
    for attr in static:
        ...
        if fitskey != 'None':
            if fitskey in header:
                ...
                if status == 1:
                    image_out_port.add_attribute(
                        attr, header[fitskey], static=True)
...
@typechecked
def set_nonstatic_attr(header: Union[fits.header.Header, Dict],
                       config_port: ConfigPort,
                       image_out_port: OutputPort,
                       check: bool = True,
                       instrument_key: Optional[str] = "default") -> None:
...
def set_extra_attr(fits_file: str,
                   ...
                   image_out_port: OutputPort,
                   first_index: int,
                   optional_attrs: Optional[list] = [],
                   instrument_key: Optional[str] = "default") -> int:
    """
